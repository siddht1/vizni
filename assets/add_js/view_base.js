app.views.TransitShowView = Backbone.View.extend({
  ...
  addRectStyles: function addRectStyles(rects, options){
  ...
  addLabelStyles: function addLabelStyles(labels, options){
  ...
  addLineStyles: function addLineStyles(lines, options){
  ...
  adjustHeight: function adjustHeight(height, stationCount, options) {
  ...
  adjustWidth: function adjustWidth(width, stationCount, options){
  ...
  drawDots: function drawDots(svg, dots) {
    svg.selectAll("dot")
      .data(dots)
      .enter().append("circle")
  ...
  drawRects: function drawRects(svg, rects){
  ...
  drawLabels: function drawLabels(svg, labels, options) {        
    svg.selectAll("text")
      .data(labels)
      .enter().append("text")
  ...
  drawLines: function drawLines(svg, lines, options) {
    var that = this,
  ...
  drawMap: function drawMap(lines, legend, width, height, options){
  ...
  getColor: function getColor(lines, colors){
    var i = lines.length;
    if (i>=colors.length) {
      i = i % lines.length;
    }
  ...
  getLengths: function getLengths(xDiff, yDiff, directions, y, options) {
    var lengths = [],
  ...
  getNextX: function getNextX(boundaries, iterator, totalPoints, width, minXDiff, prevPoint){
    var x = 0,
        prevPadding = 0.25,
        trendPadding = 0.4,
  ...
  getPointsBetween: function getPointsBetween(p1, p2, pathTypes, cornerRadius, options) {
    var that = this,
        points = [],
        x1 = p1.x, y1 = p1.y,
        x2 = p2.x, y2 = p2.y,
  ...
  getSymbol: function getSymbol(lineLabel, lines) {
  ...
  getTitleLines: function getTitleLines(title, titleMaxLineChars) {
    var lines = [],
  ...
  makeEndLines: function makeEndLines(lines, options){
  ...
  makeLegend: function makeLegend(width, lines, options){    
    var // options
        canvasWidth = width,
  ...
  makeLines: function makeLines(stations, width, height, options){
    var that = this,
        // options
        paddingX = options.padding[0],
        paddingY = options.padding[1],
  ...
  processStations: function processStations(stations)
  {
    var that = this,
  ...
  renderMap: function renderMap(options){
  ...
  translateCoordinates: function translateCoordinates(x, y, direction, length){
